...
wt_lookup_by_age <- function (id, age_1, df) {
  ...
  wt <- approx(df$age, df$weight, xout = age_1, ties = "ordered")$y
  lower <- approx(df$age, df$weight_lower, xout = age_1, ties = "ordered")$y
  upper <- approx(df$age, df$weight_upper, xout = age_1, ties = 'ordered')$y
  x = c(wt, lower, upper)
